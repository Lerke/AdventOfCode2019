...
let rec calculateNext (machineState: Machine) =
    ...
    | (OperationType.Store, modes) ->
        ...
        let input = readInputFromConsole()
        calculateNext { memory = updateMemoryLocation memory (o1) (input)
                        ip = (ip + 2) }
...
//                        | [| path |] -> readInput path
//                        | _ -> failwithf "Usage: dotnet run <path-to-input-file>"
    ...
    let initialState = { memory = input
                         ip = 0 }
    let resultingMachine = doCalculation initialState
    0
